 ...
 *
 * 环境变量:
 *   YINGSHENG_V2_COOKIE: 影声Cookie，多个账号用 &、\n 或 @ 分隔
 *
 * 参考文档: https://www.yingsheng.com/
 */

const { Env } = require('../../utils');

const $ = new Env('影声签到', { sep: ['@', '\n', '&'] });
...
class YingshengV2 {
  ...
  async signIn() {
    try {
      const response = await $.req.get('https://www.yingsheng.com/api/user/sign', {
        headers: {
          'Cookie': this.cookie,
...
$.init(YingshengV2, 'YINGSHENG_V2_COOKIE')
  .catch(error => {
