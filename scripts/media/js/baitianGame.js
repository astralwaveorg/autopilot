 ...
 *
 * 环境变量:
 *   BAITIANGAME_COOKIE: 百天游戏Cookie，多个账号用 &、\n 或 @ 分隔
 *
 * 参考文档: https://www.baitiangame.com/
 */

const { Env } = require('../../utils');

const $ = new Env('百天游戏签到', { sep: ['@', '\n', '&'] });
...
class BaitianGame {
  ...
  async signIn() {
    try {
      const response = await $.req.get('https://www.baitiangame.com/api/user/sign', {
        headers: {
          'Cookie': this.cookie,
...
$.init(BaitianGame, 'BAITIANGAME_COOKIE')
  .catch(error => {
