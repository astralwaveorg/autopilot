 ...
 *
 * 环境变量:
 *   JINGCAIZHAOPIN_COOKIE: 精彩招聘Cookie，多个账号用 &、\n 或 @ 分隔
 *
 * 参考文档: https://www.jingcaizhaopin.com/
 */

const { Env } = require('../../utils');

const $ = new Env('精彩招聘签到', { sep: ['@', '\n', '&'] });

class Jingcaizhaopin {
  ...
  async signIn() {
    try {
      const response = await $.req.get('https://www.jingcaizhaopin.com/api/user/sign', {
        headers: {
          'Cookie': this.cookie,
...
$.init(Jingcaizhaopin, 'JINGCAIZHAOPIN_COOKIE')
  .catch(error => {
