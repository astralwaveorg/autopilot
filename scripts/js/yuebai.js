 ...
 *
 * 环境变量:
 *   YUEBAI_COOKIE: 月白Cookie，多个账号用 &、\n 或 @ 分隔
 *
 * 参考文档: https://www.yuebai.com/
 */

const { Env } = require('../../utils');

const $ = new Env('月白签到', { sep: ['@', '\n', '&'] });
...
class Yuebai {
  ...
  async signIn() {
    try {
      const response = await $.req.get('https://www.yuebai.com/api/user/sign', {
        headers: {
          'Cookie': this.cookie,
