 ...
 *
 * 环境变量:
 *   DIANXIN_COOKIE: 电信Cookie，多个账号用 &、\n 或 @ 分隔
 *
 * 参考文档: https://www.189.cn/
 */

const { Env } = require('../../utils');

const $ = new Env('电信签到', { sep: ['@', '\n', '&'] });
...
class Dianxin {
  ...
  async signIn() {
    try {
      const response = await $.req.get('https://www.189.cn/api/user/sign', {
        headers: {
          'Cookie': this.cookie,
