 ...
 *
 * 环境变量:
 *   LJFSJLB_COOKIE: 龙江健身Cookie，多个账号用 &、\n 或 @ 分隔
 *
 * 参考文档: https://www.ljfsjlb.com/
 */

const { Env } = require('../../utils');

const $ = new Env('龙江健身签到', { sep: ['@', '\n', '&'] });
...
class Ljfsjlb {
  ...
  async signIn() {
    try {
      const response = await $.req.get('https://www.ljfsjlb.com/api/user/sign', {
        headers: {
          'Cookie': this.cookie,
