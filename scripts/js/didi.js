 ...
 * @Version: 1.0.0
 * * 环境变量：
 * DIDI_COOKIE: 滴滴Cookie，多账号支持分隔符：@ 或 换行 或 &
 * * 示例：
 * DIDI_COOKIE="ticket=xxx@ticket=yyy"
 */
...
const CONFIG = {
  ...
  splitters: ['@', '\n', '&'],
  timeout: 10000,
  baseUrl: 'https://www.didiglobal.com/api',
};
...
class Didi {
    ...
    try {
      ...
      if (res && res.code === 0) {
        ...
        if (reward) {
          ...
          this.log('签到成功: 今日任务已达成');
        }
        return true;
      } else {
        this.log(`签到失败: ${res?.message || '接口返回异常'}`, 'error');
        return false;
      }
    } catch (error) {
      const msg = error.response ? `HTTP ${error.response.status}` : error.message;
      this.log(`网络访问故障: ${msg}`, 'error');
      return false;
    }
  }
  ...
  async run() {
    if (!this.cookie) {
      this.log('未能获取到有效的Cookie配置', 'error');
      return;
    }
    await this.signIn();
  }
...
async function main() {
  ...
  if (!rawData) {
    console.log(`[ERROR] 环境变量 ${CONFIG.key} 未设置，请在面板中配置。`);
    return;
  }
  ...
  console.log(`[INFO] 滴滴签到脚本启动，共发现 ${cookies.length} 个账号\n`);

  for (let i = 0; i < cookies.length; i++) {
    const task = new Didi(cookies[i], i);
    await task.run();
  }
}

main()
  .catch((err) => console.error(`[CRITICAL] 脚本非正常退出: ${err.message}`))
  .finally(() => console.log('\n[INFO] 所有签到流程执行完毕'));
