 ...
 *
 * 环境变量:
 *   SYTT_COOKIE: 商通通Cookie，多个账号用 &、\n 或 @ 分隔
 *
 * 参考文档: https://www.sytt.com/
 */

const { Env } = require('../../utils');

const $ = new Env('商通通签到', { sep: ['@', '\n', '&'] });
...
class Sytt {
  ...
  async signIn() {
    try {
      const response = await $.req.get('https://www.sytt.com/api/user/sign', {
        headers: {
          'Cookie': this.cookie,
