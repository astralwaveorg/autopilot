 ...
 * @Version: 1.0.0
 * * 环境变量：
 * DFFX_COOKIE: 东方飞讯Cookie，多账号支持分隔符：@ 或 换行 或 &
 * * 示例：
 * DFFX_COOKIE="session=xxx@session=yyy"
 */
...
const CONFIG = {
  ...
  splitters: ['@', '\n', '&'],
  timeout: 10000,
  baseUrl: 'https://www.dffx.com/api',
};
...
async function main() {
  ...
  console.log(`[INFO] 东方飞讯任务开始，共检测到 ${cookies.length} 个账号\n`);

  for (let i = 0; i < cookies.length; i++) {
    const task = new Dffx(cookies[i], i);
    await task.run();
  }
}

main()
  .catch((err) => console.error(`[CRITICAL] 脚本执行中断: ${err.message}`))
  .finally(() => console.log('\n[INFO] 任务执行完毕'));
