 ...
 *
 * 环境变量:
 *   DDSY_SONGYAO_COOKIE: 叮当送药Cookie，多个账号用 &、\n 或 @ 分隔
 *
 * 参考文档: https://www.ddsy.com/
 */

const { Env } = require('../../utils');

const $ = new Env('叮当送药签到', { sep: ['@', '\n', '&'] });
...
class DdsySongyao {
  ...
  async signIn() {
    try {
      const response = await $.req.get('https://www.ddsy.com/api/user/sign', {
        headers: {
          'Cookie': this.cookie,
...
$.init(DdsySongyao, 'DDSY_SONGYAO_COOKIE')
  .catch(error => {
