 ...
 *
 * 环境变量:
 *   HAITIANMEIWEIGUAN_COOKIE: 海天美味馆Cookie，多个账号用 &、\n 或 @ 分隔
 *
 * 参考文档: https://www.haitianmeiweiguan.com/
 */

const { Env } = require('../../utils');

const $ = new Env('海天美味馆签到', { sep: ['@', '\n', '&'] });

class Haitianmeiweiguan {
  ...
  async signIn() {
    try {
      const response = await $.req.get('https://www.haitianmeiweiguan.com/api/user/sign', {
        headers: {
          'Cookie': this.cookie,
...
$.init(Haitianmeiweiguan, 'HAITIANMEIWEIGUAN_COOKIE')
  .catch(error => {
