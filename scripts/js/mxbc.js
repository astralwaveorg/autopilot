 ...
 *
 * 环境变量:
 *   MXBC_COOKIE: 美币城Cookie，多个账号用 &、\n 或 @ 分隔
 *
 * 参考文档: https://www.mxbc.com/
 */

const { Env } = require('../../utils');

const $ = new Env('美币城签到', { sep: ['@', '\n', '&'] });
...
class Mxbc {
  ...
  async signIn() {
    try {
      const response = await $.req.get('https://www.mxbc.com/api/user/sign', {
        headers: {
          'Cookie': this.cookie,
