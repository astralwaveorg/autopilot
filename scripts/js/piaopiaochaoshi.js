 ...
 *
 * 环境变量:
 *   PIAOPIAOCHAOSHI_COOKIE: 飘飘超市Cookie，多个账号用 &、\n 或 @ 分隔
 *
 * 参考文档: https://www.piaopiaochaoshi.com/
 */

const { Env } = require('../../utils');

const $ = new Env('飘飘超市签到', { sep: ['@', '\n', '&'] });

class Piaopiaochaoshi {
  ...
  async signIn() {
    try {
      const response = await $.req.get('https://www.piaopiaochaoshi.com/api/user/sign', {
        headers: {
          'Cookie': this.cookie,
...
$.init(Piaopiaochaoshi, 'PIAOPIAOCHAOSHI_COOKIE')
  .catch(error => {
