 ...
 *
 * 环境变量:
 *   SHANGSHABAN_COOKIE: 上上班Cookie，多个账号用 &、\n 或 @ 分隔
 *
 * 参考文档: https://www.shangshaban.com/
 */

const { Env } = require('../../utils');

const $ = new Env('上上班签到', { sep: ['@', '\n', '&'] });
...
class Shangshaban {
  ...
  async signIn() {
    try {
      const response = await $.req.get('https://www.shangshaban.com/api/user/sign', {
        headers: {
          'Cookie': this.cookie,
...
$.init(Shangshaban, 'SHANGSHABAN_COOKIE')
  .catch(error => {
