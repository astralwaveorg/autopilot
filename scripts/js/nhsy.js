 ...
 *
 * 环境变量:
 *   NHSY_COOKIE: 农商银行Cookie，多个账号用 &、\n 或 @ 分隔
 *
 * 参考文档: https://www.nhsy.com/
 */

const { Env } = require('../../utils');

const $ = new Env('农商银行签到', { sep: ['@', '\n', '&'] });
...
class Nhsy {
  ...
  async signIn() {
    try {
      const response = await $.req.get('https://www.nhsy.com/api/user/sign', {
        headers: {
          'Cookie': this.cookie,
