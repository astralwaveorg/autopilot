 ...
 *
 * 环境变量:
 *   ZHIWUXINGQIU_COOKIE: 植物星球Cookie，多个账号用 &、\n 或 @ 分隔
 *
 * 参考文档: https://www.zhiwuxingqiu.com/
 */

const { Env } = require('../../utils');

const $ = new Env('植物星球签到', { sep: ['@', '\n', '&'] });
...
class Zhiwuxingqiu {
  ...
  async signIn() {
    try {
      const response = await $.req.get('https://www.zhiwuxingqiu.com/api/user/sign', {
        headers: {
          'Cookie': this.cookie,
...
$.init(Zhiwuxingqiu, 'ZHIWUXINGQIU_COOKIE')
  .catch(error => {
