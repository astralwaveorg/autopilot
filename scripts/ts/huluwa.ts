 ...
 *
 * 环境变量:
 *   HULUWA_COOKIE: 葫芦娃Cookie，多个账号用 &、\n 或 @ 分隔
 *
 * 参考文档: https://www.huluwa.com/
 */

import { Env } from '../utils';

const $ = new Env('葫芦娃签到', { sep: ['@', '\n', '&'] });
...
class Huluwa {
  ...
  async signIn(): Promise<boolean> {
    try {
      const { data: res } = await $.req.get('https://www.huluwa.com/api/user/sign', {
        headers: {
          'Cookie': this.cookie,
