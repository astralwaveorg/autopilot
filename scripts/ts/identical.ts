 ...
 *
 * 环境变量:
 *   IDENTICAL_COOKIE: 相同Cookie，多个账号用 &、\n 或 @ 分隔
 *
 * 参考文档: https://www.identical.com/
 */

import { Env } from '../utils';

const $ = new Env('相同签到', { sep: ['@', '\n', '&'] });
...
class Identical {
  ...
  async signIn(): Promise<boolean> {
    try {
      const { data: res } = await $.req.get('https://www.identical.com/api/user/sign', {
        headers: {
          'Cookie': this.cookie,
...
$.init(Identical, 'IDENTICAL_COOKIE')
  .catch(error => {
