...
环境变量:
   CHANGHONGMEILING_COOKIE: 长虹美菱Cookie，多个账号用 &、\n 或 @ 分隔

参考文档: https://www.changhongmeiling.com/
"""
    ...
    def sign_in(self) -> bool:
        ...
        try:
            response = requests.get(
                'https://www.changhongmeiling.com/api/user/sign',
                headers={
                    'Cookie': self.cookie,
...
def main():
    """主函数"""
    env_value = os.getenv('CHANGHONGMEILING_COOKIE', '')

    if not env_value:
        print('未找到环境变量 CHANGHONGMEILING_COOKIE')
        return
    ...
    for idx, user_config in enumerate(users):
        task = Changhongmeiling(user_config, idx)
        task.start()
