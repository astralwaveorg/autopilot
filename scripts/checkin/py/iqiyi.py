...
环境变量:
   IQIYI_COOKIE: 爱奇艺Cookie，多个账号用 &、\n 或 @ 分隔

参考文档: https://www.iqiyi.com/
"""
    ...
    def sign_in(self) -> bool:
        ...
        try:
            response = requests.get(
                'https://tc.vip.iqiyi.com/taskCenter/task/queryUserTask',
                headers={
                    'Cookie': self.cookie,
            ...
            if res.get('code') == 'A00000' and res.get('data'):
...
def main():
    """主函数"""
    env_value = os.getenv('IQIYI_COOKIE', '')

    if not env_value:
        print('未找到环境变量 IQIYI_COOKIE')
        return
