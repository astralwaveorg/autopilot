 ...
 *
 * 环境变量:
 *   AIMA_COOKIE: 爱码Cookie，多个账号用 &、\n 或 @ 分隔
 *
 * 参考文档: https://www.aima.com/
 */

import { Env } from '../utils';

const $ = new Env('爱码签到', { sep: ['@', '\n', '&'] });
...
class Aima {
  ...
  async signIn(): Promise<boolean> {
    try {
      const { data: res } = await $.req.get('https://www.aima.com/api/user/sign', {
        headers: {
          'Cookie': this.cookie,
