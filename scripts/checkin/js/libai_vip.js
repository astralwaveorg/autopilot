 ...
 *
 * 环境变量:
 *   LIBAIVIP_COOKIE: 立白VIPCookie，多个账号用 &、\n 或 @ 分隔
 *
 * 参考文档: https://www.libai.com/
 */

const { Env } = require('../../utils');

const $ = new Env('立白VIP签到', { sep: ['@', '\n', '&'] });
...
class LibaiVIP {
  ...
  async signIn() {
    try {
      const response = await $.req.get('https://www.libai.com/api/user/sign', {
        headers: {
          'Cookie': this.cookie,
