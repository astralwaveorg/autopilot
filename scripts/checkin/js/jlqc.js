 ...
 *
 * 环境变量:
 *   JLQC_COOKIE: 金龙汽车Cookie，多个账号用 &、\n 或 @ 分隔
 *
 * 参考文档: https://www.jlqc.com/
 */

const { Env } = require('../../utils');

const $ = new Env('金龙汽车签到', { sep: ['@', '\n', '&'] });
...
class Jlqc {
  ...
  async signIn() {
    try {
      const response = await $.req.get('https://www.jlqc.com/api/user/sign', {
        headers: {
          'Cookie': this.cookie,
