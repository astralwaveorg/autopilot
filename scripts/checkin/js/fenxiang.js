 ...
 *
 * 环境变量:
 *   FENXIANG_COOKIE: 分享Cookie，多个账号用 &、\n 或 @ 分隔
 *
 * 参考文档: https://www.fenxiang.com/
 */

const { Env } = require('../../utils');

const $ = new Env('分享签到', { sep: ['@', '
', '&'] });
...
class Fenxiang {
  ...
  async signIn() {
    try {
      const response = await $.req.get('https://www.fenxiang.com/api/user/sign', {
        headers: {
          'Cookie': this.cookie,
