 ...
 *
 * 环境变量:
 *   KSJSB_COOKIE: 快捷记账Cookie，多个账号用 &、\n 或 @ 分隔
 *
 * 参考文档: https://www.ksjsb.com/
 */

const { Env } = require('../../utils');

const $ = new Env('快捷记账签到', { sep: ['@', '\n', '&'] });
...
class Ksjsb {
  ...
  async signIn() {
    try {
      const response = await $.req.get('https://www.ksjsb.com/api/user/sign', {
        headers: {
          'Cookie': this.cookie,
