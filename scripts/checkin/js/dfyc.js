 ...
 *
 * 环境变量:
 *   DFYC_COOKIE: 东方云彩Cookie，多个账号用 &、\n 或 @ 分隔
 *
 * 参考文档: https://www.dfyc.com/
 */

const { Env } = require('../../utils');

const $ = new Env('东方云彩签到', { sep: ['@', '\n', '&'] });
...
class Dfyc {
  ...
  async signIn() {
    try {
      const response = await $.req.get('https://www.dfyc.com/api/user/sign', {
        headers: {
          'Cookie': this.cookie,
