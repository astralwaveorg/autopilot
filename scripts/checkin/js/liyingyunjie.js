 ...
 * @Author: Astral
 * @Date: 2025-01-28
 * @Description: liyingyunjie.js - checkin
 * @Source: AutoPilot
 *
 * 
 *
...
$.appid = 'wx3aa4511d6dfda7ce';  //  appId
$.messages = [];
...
async function main() {
  ...
  //   await getToken($.codeList[i]);  //  Token
  // }
 $.log(userArr);
//   if ($.userArr.length) {
        if (userArr[0]) {

    $.log(` ${userArr.length}  Token  `);
    ...
    //   $.member_id = $.idArr[i]['member_id'];
      $.member_id = idArr[i];
      $.token = 'Bearer ' + userArr[i];  //  Bearer
    //   $.token = $.userArr[i]['access_token'].startsWith('Bearer ') ? $.userArr[i]['access_token'] : 'Bearer ' + $.userArr[i]['access_token'];  //  Bearer

      // 
      await whoami();

      if (!$.is_login) continue;  //  token 
...
async function checkin() {
  ...
  if (result?.code == 200 && result?.data) {
    msg += `:  ${result.data.num} ,  +${result.data.point} `;
  } else {
    msg += `: , ${result?.msg || $.toStr(result)}`;
  }
 ...
 * 
 * @param {string} string - 
 * @param {number} head_length -  2
 * @param {number} foot_length -  2
 * @returns {string} - 
 */
 ...
 * @returns {(object|string)} -  options['respType']  {status|headers|rawBody}  body
 */
async function Request(options) {
  try {
    ...
    const _http = [
      new Promise((_, reject) => setTimeout(() => reject(`  ${options['url']}`), _timeout)),
