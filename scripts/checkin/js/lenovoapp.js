 ...
 *
 * 环境变量:
 *   LENOVOAPP_COOKIE: 联想Cookie，多个账号用 &、\n 或 @ 分隔
 *
 * 参考文档: https://www.lenovo.com/
 */

const { Env } = require('../../utils');

const $ = new Env('联想签到', { sep: ['@', '\n', '&'] });
...
class Lenovoapp {
  ...
  async signIn() {
    try {
      const response = await $.req.get('https://www.lenovo.com/api/user/sign', {
        headers: {
          'Cookie': this.cookie,
...
$.init(Lenovoapp, 'LENOVOAPP_COOKIE')
  .catch(error => {
