 ...
 *
 * 环境变量:
 *   XIAOSATONGLU_COOKIE: 小沙通路Cookie，多个账号用 &、\n 或 @ 分隔
 *
 * 参考文档: https://www.xiaosatonglu.com/
 */

const { Env } = require('../../utils');

const $ = new Env('小沙通路签到', { sep: ['@', '\n', '&'] });
...
class Xiaosatonglu {
  ...
  async signIn() {
    try {
      const response = await $.req.get('https://www.xiaosatonglu.com/api/user/sign', {
        headers: {
          'Cookie': this.cookie,
...
$.init(Xiaosatonglu, 'XIAOSATONGLU_COOKIE')
  .catch(error => {
