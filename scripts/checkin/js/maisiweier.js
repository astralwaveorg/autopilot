 ...
 *
 * 环境变量:
 *   MAISIWEIER_COOKIE: 麦斯韦尔Cookie，多个账号用 &、\n 或 @ 分隔
 *
 * 参考文档: https://www.maisiweier.com/
 */

const { Env } = require('../../utils');

const $ = new Env('麦斯韦尔签到', { sep: ['@', '\n', '&'] });
...
class Maisiweier {
  ...
  async signIn() {
    try {
      const response = await $.req.get('https://www.maisiweier.com/api/user/sign', {
        headers: {
          'Cookie': this.cookie,
...
$.init(Maisiweier, 'MAISIWEIER_COOKIE')
  .catch(error => {
