 ...
 *
 * 环境变量:
 *   SHANGZHANWANGLUO_COOKIE: 商战网络Cookie，多个账号用 &、\n 或 @ 分隔
 *
 * 参考文档: https://www.shangzhanwangluo.com/
 */

const { Env } = require('../../utils');

const $ = new Env('商战网络签到', { sep: ['@', '\n', '&'] });

class Shangzhanwangluo {
  ...
  async signIn() {
    try {
      const response = await $.req.get('https://www.shangzhanwangluo.com/api/user/sign', {
        headers: {
          'Cookie': this.cookie,
...
$.init(Shangzhanwangluo, 'SHANGZHANWANGLUO_COOKIE')
  .catch(error => {
