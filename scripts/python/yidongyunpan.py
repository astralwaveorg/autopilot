...
环境变量:
   YIDONGYUNPAN_TOKEN: 移动云盘Token，多个账号用 &、\n 或 @ 分隔

参考文档: https://www.yidongyunpan.com/
"""
    ...
    def sign_in(self) -> bool:
        ...
        try:
            response = requests.get(
                'https://www.yidongyunpan.com/api/user/sign',
                headers={
...
def main():
    """主函数"""
    env_value = os.getenv('YIDONGYUNPAN_TOKEN', '')

    if not env_value:
        print('未找到环境变量 YIDONGYUNPAN_TOKEN')
        return
    ...
    for idx, user_config in enumerate(users):
        task = Yidongyunpan(user_config, idx)
        task.start()
