...
环境变量:
   QUANZHAN_COOKIE: 全栈Cookie，多个账号用 &、\n 或 @ 分隔

参考文档: https://www.quanzhan.com/
"""
    ...
    def sign_in(self) -> bool:
        ...
        try:
            response = requests.get(
                'https://www.quanzhan.com/api/user/sign',
                headers={
                    'Cookie': self.cookie,
            ...
            if res.get('code') == 0 and res.get('data'):
                ...
                print(f'账号 {this.index + 1} 签到失败: {res.get("message", "未知错误")}')
                return False
...
def main():
    """主函数"""
    env_value = os.getenv('QUANZHAN_COOKIE', '')

    if not env_value:
        print('未找到环境变量 QUANZHAN_COOKIE')
        return
