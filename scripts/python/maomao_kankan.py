...
环境变量:
   MAOMAOKANKAN_COOKIE: 猫猫看看Cookie，多个账号用 &、\n 或 @ 分隔

参考文档: https://www.maomaokankan.com/
"""
    ...
    def sign_in(self) -> bool:
        ...
        try:
            response = requests.get(
                'https://www.maomaokankan.com/api/user/sign',
                headers={
                    'Cookie': self.cookie,
...
def main():
    """主函数"""
    env_value = os.getenv('MAOMAOKANKAN_COOKIE', '')

    if not env_value:
        print('未找到环境变量 MAOMAOKANKAN_COOKIE')
        return
    ...
    for idx, user_config in enumerate(users):
        task = Maomaokankan(user_config, idx)
        task.start()
