 ...
 *
 * 环境变量:
 *   JLYH_COOKIE: 吉林银行Cookie，多个账号用 &、\n 或 @ 分隔
 *
 * 参考文档: https://www.jlyh.com/
 */

const { Env } = require('../../utils');

const $ = new Env('吉林银行签到', { sep: ['@', '\n', '&'] });
...
class Jlyh {
  ...
  async signIn() {
    try {
      const response = await $.req.get('https://www.jlyh.com/api/user/sign', {
        headers: {
          'Cookie': this.cookie,
